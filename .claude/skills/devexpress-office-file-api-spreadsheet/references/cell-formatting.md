...
Apply different fonts, sizes, or colors to individual parts of a cell value using `RichTextString`:
...
// Build rich text from separate runs — each with its own font settings
RichTextString richText = new RichTextString();
richText.AddTextRun("Total: ",  new RichTextRunFont("Calibri",  12, Color.Black));
richText.AddTextRun("$1,500",   new RichTextRunFont("Calibri",  12, Color.DarkGreen));
richText.AddTextRun(" (Q1)",    new RichTextRunFont("Calibri",  10, Color.Gray));

sheet["A1"].SetRichText(richText);
```

To apply formatting to a portion of an existing cell value, use `Characters(startIndex, length)`:

```csharp
// Set the cell value and base font first
Cell cell = sheet["B2"];
cell.Value = "Rich text formatting";
cell.Font.Name = "Arial";
cell.Font.Size = 14;

// Get the RichTextString and preserve the base font across all characters
RichTextString richText = cell.GetRichText();
richText.Characters().SetFont(cell.Font);

// Bold the first word only
richText.Characters(0, 4).Font.Bold = true;

cell.SetRichText(richText);
```

Key types: `RichTextString`, `RichTextRunFont`. Use `Cell.SetRichText` to assign and `Cell.GetRichText` to read back.
