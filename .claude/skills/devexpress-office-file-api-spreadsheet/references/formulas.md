...
## Dynamic Array Formulas (v26.1+)

Dynamic array formulas spill results into adjacent cells automatically. Assign them without the leading `=` to `DynamicArrayFormula` (localized) or `DynamicArrayFormulaInvariant` (portable):

```csharp
// Assign a dynamic array formula — results spill into adjacent cells
worksheet["A1"].DynamicArrayFormula = "SORT(C1:C10)";
workbook.Calculate();

// Read the spilled range
CellRange spillRange = worksheet["A1"].GetSpillRange();
foreach (Cell cell in spillRange)
    Console.WriteLine(cell.Value);
```

### Supported Dynamic Array Functions (v26.1+)

| Function | Description |
|----------|-------------|
| `XLOOKUP` | Search a range and return a full row/column result |
| `XMATCH` | Return the relative position of a value in a range |
| `SORT` | Sort a range or array and return sorted results |
| `SORTBY` | Sort by values in a separate range |
| `FILTER` | Return rows meeting a condition |
| `UNIQUE` | Extract distinct values from a range |

Legacy functions like `LEN` and `TRANSPOSE` also behave as dynamic array formulas when their result naturally spills across multiple cells.

Use `worksheet.DynamicArrayFormulas.Add(cell, formula)` to add from a collection, or assign directly to the cell as shown above.
