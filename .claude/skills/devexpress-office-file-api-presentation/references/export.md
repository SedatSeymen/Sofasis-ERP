...
Be aware of the following known limitations:
...
- PDF/A compliance mode is not available.
- Certain combinations of `ModificationPermissions` and `InteractivityPermissions` are not supported.

## Export Slides to Images (v26.1+)

Export all slides or a specific subset to raster or vector images with full visual fidelity (themes, backgrounds, shapes).
...
// Export all slides to PNG files
...
Using presentation As New Presentation(File.ReadAllBytes("Presentation.pptx"))
    Dim images As DXImage() = presentation.ExportToImages()
...
```csharp
// Export only slides 0 and 2 (zero-based index)
DXImage[] selected = presentation.ExportToImages(0, 2);
```

### Configure Resolution and Rendering

```csharp
var options = new ImageExportOptions
{
    Resolution = 150,    // DPI (default: 96)
    Rasterize = true     // Raster rendering (default: true)
};
...
**Note:** Printing in non-Windows environments (macOS, Linux) requires the `libcups2` package (CUPS). Ensure it is installed separately.
