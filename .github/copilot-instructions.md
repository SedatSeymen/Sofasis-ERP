...
- **Faz A durumu:** Kopyala+yükselt TAMAM — 26.1.3 / .NET 10 ile **derleniyor ve çalışıyor**. Notlar: `DistributedIdGeneratorHelper` 26.1'de kalktığı için yerel drop-in eklendi (`BusinessObjects/Generators/`); `DatabaseUpdate/DatabaseSeed.cs` geçici olarak derleme dışı (eski base'e göre `CreateUserID/UpdateUserID` uyumsuz — doğru base'e uyarlanıp `Updater`'a bağlanacak). Sıradaki: seed'i geri kazan + Blazor host navigasyonu (`Model.xafml`) taşı; sonra Faz B (Guid).
- **PK — Faz A'da mevcut `KeyID` string(13) KORUNUR.** Guid'e geçiş **Faz B**'de, izole ve test edilerek yapılacak (ŞİMDİ DEĞİL). Yani §2/§3'teki "Guid PK" kararı Faz B'ye ertelendi; şu an KeyID ile çalış.
...
- **Sıra kuralı:** Yükseltme (DX+net) ile PK migration'ı **aynı anda yapma** — önce yükselt+stabilize (KeyID), sonra Faz B. Hata izolasyonu için şart.
